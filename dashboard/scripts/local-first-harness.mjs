...
try {
  ...
  assert.match(peopleWorkspace, /Select an organization/i);
  assert.match(peopleWorkspace, /placeholder="Start typing…"/i);
  ...
  const styleGuideWorkspace = await readFile("components/personal-ops/PersonalStyleGuideWorkspace.tsx", "utf8");
  assert.match(styleGuideWorkspace, /FONT_PREVIEW_STACKS/i);

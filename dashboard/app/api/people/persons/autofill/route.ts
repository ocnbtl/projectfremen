...
import { discoverPerson } from "../../../../../lib/server/person-discovery";
...
export async function POST(request: Request) {
  ...
  if (!isCsrfRequestValid(request)) {
    await appendAuditEvent({ at: new Date().toISOString(), action: "people.person.autofill.csrf_failed", path: new URL(request.url).pathname, method: "POST", ip: getRequestIp(request), status: "denied" });
  ...
  try {
    ...
    name = body.name?.trim() || ""; if (!name) throw new Error();
    urls = links.map((url: string) => normalizeOrganizationUrl(url));
  } catch { return json({ ok: false, error: "Enter the person’s name and a public HTTP or HTTPS profile link." }, 400); }
  const now = Date.now();
  ...
  try {
    return json({ ok: true, result: await discoverPerson(name, urls) });
  } catch (error) {
    ...
    const message = error instanceof Error && /^(Use a public|This (link|website|page)|The website took|This website did|This page is)/.test(error.message)
      ? error.message : "This website could not be read. Try the person’s website or public biography or enter its details yourself.";

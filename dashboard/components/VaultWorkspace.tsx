...
const VAULT_RELEASE_NOTE = "devices-at-a-glance-v1";
...
export default function VaultWorkspace() {
  ...
  async function createBackup() {
    await run(async () => {
      await browserVault.createDesktopBackup();
      setMessage("Backup created on this PC.");
    });
  }
  ...
  async function saveLocalObject() {
    await run(async () => {
      ...
      await loadObjects();
      setMessage(`${activeKind === "contact" ? "Contact" : activeKind[0].toUpperCase() + activeKind.slice(1)} saved. It will sync automatically.`);
    });
  }
  ...
  return (
    <main className={styles.page}>
      ...
      {status?.configured && showReleaseNote && <aside className={styles.releaseNote} aria-label="What is new">
        <div><strong>Your devices are now easier to see.</strong><span>They appear at the top of the Vault after you unlock it.</span></div>
        ...
        <>
          ...
          <section className={styles.grid}>
            ...
            <article className={styles.panel}>
              ...
              <small>Keep the recovery file somewhere separate from your password. You can add an external drive or home server later.</small>
            </article>
          </section>
          ...
          <section className={`${styles.panel} ${styles.objectWorkspace}`}>
            <div className={styles.workspaceHeader}>
              <div><p className={styles.eyebrow}>Saved on this device</p><h2>Notes, contacts, and resources</h2></div>
            ...
            <div className={styles.objectGrid}>
              <aside className={styles.objectList} aria-label={`${activeKind} list`}>
                ...
                {!objects.some((item) => item.objectKind === activeKind) && <p>No {activeKind}s on this device yet.</p>}
              </aside>

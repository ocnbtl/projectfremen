      ...
      return <section className="finance-account-group" key={group.id} aria-label={group.label}>
        ...
        <ul className="finance-account-cards">{rows.map(({ account, activity }) => {
          ...
          return <li key={account.id}><button type="button" className="finance-account-card" data-finance-account-id={account.id} aria-pressed={model.selectedId === account.id} aria-controls="finance-inspector" onClick={() => onSelect(account.id)}>
            ...
            <strong className="finance-account-balance">{money(account.balance, { cents: true })}</strong><span className="finance-account-asof">{native?.balanceSource === "coinbase" ? "Coinbase estimate" : native?.balanceSource === "plaid" ? native.kind === "Brokerage" ? "Investment balance retrieved" : "Bank balance retrieved" : native?.balanceSource === "imported" ? "Imported balance" : "Recorded balance"} · {native?.balanceAsOf.slice(0, 10) || "Date unavailable"}</span>

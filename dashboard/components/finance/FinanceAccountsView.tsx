...
export default function FinanceAccountsView({
  ...
  const reconciliationRows = model.rows.filter((row) =>
    row.activity.transactions.some((transaction) => transaction.status === "pending" || !transaction.receipt.trim())
  );
  ...
  return (
    <>
      <WorkspaceHeader
        ...
        subtitle="Explicit balance facts, paired transfers, savings movement, and reconciliation evidence"
        actions={(
          <>
      ...
      <MetricStrip
        ...
        items={[
          ...
          { id: "debt", label: "Debt", value: money(model.totals.liabilities, { cents: true }), detail: "Signed explicit liability", tone: model.totals.debtOwed ? "danger" : "default" },
          { id: "net", label: "Net worth", value: money(model.totals.net, { cents: true }), detail: "Current balance snapshots" },
          { id: "savings", label: "Savings moved", value: money(actualSavingsMovement, { sign: true, cents: true }), detail: "First-class movement evidence", tone: "positive" },
          ...
          { id: "imports", label: "Import health", value: "Counts", detail: "Accepted, ambiguous, rejected, unreconciled" }
        ]}
      />
      ...
      <div className={styles.accountLayout}>
        <Panel hue="teal">
          <div className="finance-panel-heading">
            <h2>Cashflow <span>6 mo · native ledger</span></h2>
            ...
            {reconciliationRows.length ? (
              <ul className={`${styles.evidenceList} ${styles.semanticList}`} aria-label="Accounts needing reconciliation review">
                {reconciliationRows.map((row) => {
                  const issues = row.activity.transactions.filter((transaction) => transaction.status === "pending" || !transaction.receipt.trim());
                  return (
          ...
          <Panel hue="violet">
            ...
            <div className={styles.compactList}>
              <div className={styles.compactRow}>
                <span><strong>Actual savings movement</strong><small>First-class movement facts; transfers retain separate paired identities</small></span>
      ...
      <Panel hue="blue" className={`${styles.accountInventory} finance-ledger-panel`}>
                    ...
                    <li className={styles.semanticListItem} key={account.id}>
                      ...
                      >
                        ...
                        <span className="finance-row-identity">
                          ...
                          <span className={styles.accountRowMeta}>{activity.transactions.length} transactions · {activity.bills.length} bills · resolved native account</span>
                        </span>
        ...
        {model.rows.length === 0 && (
          <SystemState
            ...
            description="Clear the query to return to the current account inventory."
